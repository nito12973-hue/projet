...
import { Heart, ShoppingCart, UserCircle2, Settings } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80">
      <div className="container-page flex min-h-20 flex-wrap items-center justify-between gap-3 py-3">
        <Link href="/" className="text-xl font-black tracking-tight text-slate-900 dark:text-white sm:text-2xl">Sunu<span className="text-[color:var(--accent-color)]">Market</span></Link>
        <nav className="hidden items-center gap-6 md:flex">
          ...
          <Link href="/settings" className="text-sm font-medium text-slate-600 hover:text-brand-500 dark:text-slate-300">{messages.navbar.settings}</Link>
        </nav>
        <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
          ...
          <Link href="/settings" className="rounded-full border border-slate-200 p-2 dark:border-slate-700" aria-label={messages.navbar.settings}><Settings size={18} /></Link>
          {user ? (
      ...
      <div className="border-t border-slate-200/70 md:hidden dark:border-slate-800">
        <div className="container-page flex gap-2 overflow-x-auto py-3 text-sm font-semibold">
          ...
          <Link href="/settings" className="whitespace-nowrap rounded-full border border-slate-200 px-4 py-2 dark:border-slate-700">{messages.navbar.settings}</Link>
        </div>
      </div>
    </header>
  );
